    ...
    internal class CallSiteFactory
    {
                ...
                if (serviceType.IsGenericTypeDefinition)
                {
                    ...
                    if (implementationType == null || !implementationType.IsGenericTypeDefinition)
                    {
                        throw new ArgumentException(
                            Strings.Format(Strings.OpenGenericServiceRequiresOpenGenericImplementation, descriptor.ServiceType),
                            "descriptors");
                    }
                    ...
                    {
                        throw new ArgumentException(
                            Strings.Format(Strings.TypeCannotBeActivated, descriptor.ImplementationType, descriptor.ServiceType));
                    }
                }
            ...
            CallSiteChain callSiteChain)
        {
            try
            {
                ...
                {
                    throw new InvalidOperationException(Strings.Format(Strings.NoConstructorMatch, implementationType));
                }
                        ...
                        else
                        {
                            ...
                            if (!bestConstructorParameterTypes.IsSupersetOf(parameters.Select(p => p.ParameterType)))
                            {
                                ...
                                string message = string.Join(
                                    Environment.NewLine,
                                    Strings.Format(Strings.AmbiguousConstructorException, implementationType),
                                    bestConstructor,
                                    constructors[i]);
                ...
                if (bestConstructor == null)
                {
                    throw new InvalidOperationException(
                        Strings.Format(Strings.UnableToActivateTypeException, implementationType));
                }
                else
                {
            ...
            bool throwIfCallSiteNotFound)
        {
            ...
            for (int index = 0; index < parameters.Length; index++)
            {
                ...
                if (callSite == null)
                {
                    ...
                    {
                        throw new InvalidOperationException(Strings.Format(Strings.CannotResolveService,
                            parameterType,
                            implementationType));
                    }
